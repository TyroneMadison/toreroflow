...
- [ ] **Step 2:** In the app, open Upload & Schedule on Caleb. Every ready card shows the Edit cover button. Open it: the video loads, the scrubber seeks, "Use this frame" saves, and the card thumbnail visibly changes to the chosen frame. Reopen: the scrubber starts at the saved offset.
...
- [ ] **Step 5:** Dry-run proof: on a client whose accounts are dry-run (JR Michael LLC or a throwaway), schedule a post to Instagram + YouTube "now" with options set and a frame cover chosen. Read the worker window's `[dryrun:instagram] publish` log: extras must show `contentType: "reels"`, `instagramThumbnail`, `trialParams { graduationStrategy: "SS_PERFORMANCE" }`, the cleaned collaborator (no @), `audioName`, `firstComment`. The `[dryrun:youtube]` log: extras show `platformSpecificData.title` and, for a short-form asset, NO mediaThumbnail; the logged caption is the description when one exists.
- [ ] **Step 6:** Clean up: delete the dry-run test post targets (calendar delete), remove any test cover, restore anything touched on Caleb.
- [ ] **Step 7:** A real Instagram publish with a trial reel to Caleb's account is Tyrone's decision, separately from this walk.
