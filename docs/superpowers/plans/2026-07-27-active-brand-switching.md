...
- [ ] **Step 2: Switch back to Northstar.** Expect: his uploads, calendar posts, and workflows all present, exactly as before.
- [ ] **Step 3: Draft leak check.** On Northstar's Upload screen, type into a title field without saving, switch to `ZZ Test Brand`, then back. Expect: the typed text is gone, and nothing of Northstar's appears under the test brand.
- [ ] **Step 4: Modal check.** Open the schedule modal on Northstar, switch brands in the sidebar. Expect: the modal is gone after the switch.
- [ ] **Step 5: Failure honesty check.** Stop the API process, switch brands. Expect: empty screens plus an error toast naming what failed, never Northstar's rows under the test brand's name. Restart the API afterward and confirm data returns on the next switch or poll.
...
- [ ] **Step 9: Clean up.** Confirm `ZZ Test Brand` and any test rows it created are gone. Northstar's data untouched.
