...
1. Onboard a test brand. Expect: it becomes active immediately, Upload,
   ...
   quota.
2. Switch back to Northstar. Expect: his uploads, calendar, and workflows exactly
   as before.
3. Start typing a title on Northstar, switch brands. Expect: the draft is gone on
...
5. Stop the API, switch brands. Expect: empty screens plus an error toast,
   never Northstar's rows under the test brand's name.
6. Delete the test brand while it is active. Expect: selection falls back to
   Northstar, screens show his data.
