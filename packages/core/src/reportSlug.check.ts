...
for (const name of ["-Northstar-", "  CACV  ", "***Bob***"]) {
...
assert.equal(buildReportSlug("Northstar").endsWith(`-${REPORT_SLUG_SUFFIX}`), true);
...
const long = buildReportSlugWithSuffix("Northstar", seq, 4);
assert.equal(long.startsWith("northstar-monthly-reports-"), true, long);
